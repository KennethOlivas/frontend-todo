...
export default function Todo() {
    ...
    return (
        <div className="flex flex-col space-y-4 min-w-screen h-screen animated fadeIn faster  left-0 top-0 mt-12 items-center inset-0 z-50 outline-none focus:outline-none bg-gray-900">
            <div className="flex flex-col p-8 bg-stone-800 shadow-md hover:shodow-lg rounded-2xl">
                ...
                <div className="flex flex-col space-y-4">
                    ...
                    <button type="submit" onClick={() => { isEditing ? editTodo(id, title, description, priority, state) : addTodo(title, description, priority) }} className={` ${isEditing ? "text-yellow-500 hover:bg-yellow-400" : "text-emerald-500 hover:bg-emerald-400"}  hover:text-white font-montserrat  py-2 px-8 font-medium rounded-xl transition-all duration-300`}>{isEditing ? "EDIT" : "ADD"}</button>
            ...
            <div className="grid grid-cols-1 mx-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 ">
                ...
                {todos.map(todo => (
                    <div onDoubleClick={() => { updateTodo(todo) }} key={todo.id} className={`relative flex-grow p-4 space-x-2 justify-center ${todo.state ? "bg-emerald-700 hover:bg-emerald-600 hover:shadow-md hover:shadow-stone-400" : "bg-stone-800 hover:bg-stone-900 hover:shadow-md hover:shadow-stone-700"}  text-white rounded-lg outline-none focus:outline-none capitalize   transition-all duration-300 cursor-pointer`}>
                        ...
                        <button onClick={() => { setEdit(todo) }} className="text-yellow-600 hover:bg-yellow-400 hover:text-white font-montserrat  py-2 px-8 font-medium rounded-xl transition-all duration-300 ">Edit</button>
                        <button onClick={() => { deleteTodo(todo.id) }} className="text-rose-600 hover:bg-rose-400 hover:text-white font-montserrat  py-2 px-8 font-medium rounded-xl transition-all duration-300 ">Delete</button>
                    </div>
                ))}
